...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    if (!HasPermission(rbac::RBAC_PERM_SKIP_CHECK_CHARACTER_CREATION_RACEMASK))
    {
        ...
        uint32 raceMask = GetRaceMaskForRace(createInfo->Race);
        if (raceMask && (raceMask & raceMaskDisabled))
        {
            SendCharCreate(CHAR_CREATE_DISABLED);
            return;
        }
...
void WorldSession::HandleCharFactionOrRaceChangeCallback(std::shared_ptr<CharacterFactionChangeInfo> factionChangeInfo, PreparedQueryResult result)
{
    ...
    if (factionChangeInfo->FactionChange)
    {
        ...
        {
            AuctionHouseObject* auctionHouse = sAuctionMgr->GetAuctionsMap(i == 0 ? 0 : ((Player::TeamIdForRace(playerData->Race) == TEAM_ALLIANCE) ? 12 : 29));
    ...
    {
        uint32 raceMaskDisabled = sWorld->getIntConfig(CONFIG_CHARACTER_CREATING_DISABLED_RACEMASK);
        uint32 raceMask = GetRaceMaskForRace(factionChangeInfo->Race);
        if (raceMask && (raceMask & raceMaskDisabled))
        {
            SendCharFactionChange(CHAR_CREATE_ERROR, factionChangeInfo.get());
            return;
